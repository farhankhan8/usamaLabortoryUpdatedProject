...
class TestsPerformedController extends Controller
{
    public function index()
    {
        $testPerformeds = DB::table('test_performeds')
            ...
            ->select('test_performeds.*', 'patients.Pname', 'patients.dob', 'available_tests.name', 'available_tests.testFee', 'categories.Cname', 'statuses.Sname')
    ...
    public function store(Request $request)
    {
        ...
        //fee
        if (isset($request->fee_final) && $request->fee_final)
            $fee=$request->fee_final;
        else{
        ...
        //        store
        // $test_performed = TestPerformed::create([
        ...
        $test_performed->available_test_id = $request->available_test_id;
        $test_performed->patient_id = $request->patient_id;
        $test_performed->Sname_id = $request->Sname_id;
        $test_performed->type = $request->type;
        $test_performed->fee = $fee;

        if (!empty($request->input('comments')))
        {
            $test_performed->comments = $request->comments;
        }
        else {
